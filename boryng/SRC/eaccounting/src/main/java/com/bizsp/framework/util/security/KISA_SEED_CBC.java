...
public class KISA_SEED_CBC {
	...
	private static final int SS3[] = { 0x08303838, 0xc8e0e828, 0x0d212c2d,
	...
	private static final void SeedRound(int[] T, int LR[], int L0, int L1,
			int R0, int R1, int[] K, int K_offset) {
	...
	private static final void RoundKeyUpdate0(int T[], int[] K, int K_offset,
			int ABCD[], int KC) {
		...
		K[K_offset + 1] = SS0[GetB0(T[1]) & 0x0ff] ^ SS1[GetB1(T[1]) & 0x0ff]
				^ SS2[GetB2(T[1]) & 0x0ff] ^ SS3[GetB3(T[1]) & 0x0ff];
		T[0] = ABCD[ABCD_A];
		ABCD[ABCD_A] = ((ABCD[ABCD_A] >> 8) & 0x00ffffff)
				^ (ABCD[ABCD_B] << 24);
		ABCD[ABCD_B] = ((ABCD[ABCD_B] >> 8) & 0x00ffffff) ^ (T[0] << 24);
	}

	private static final void RoundKeyUpdate1(int T[], int[] K, int K_offset,
			int ABCD[], int KC) {
		T[0] = ABCD[ABCD_A] + ABCD[ABCD_C] - KC;
		T[1] = ABCD[ABCD_B] + KC - ABCD[ABCD_D];
		K[K_offset + 0] = SS0[GetB0(T[0]) & 0x0ff] ^ SS1[GetB1(T[0]) & 0x0ff]
		...
		K[K_offset + 1] = SS0[GetB0(T[1]) & 0x0ff] ^ SS1[GetB1(T[1]) & 0x0ff]
				^ SS2[GetB2(T[1]) & 0x0ff] ^ SS3[GetB3(T[1]) & 0x0ff];
		T[0] = ABCD[ABCD_C];
		ABCD[ABCD_C] = (ABCD[ABCD_C] << 8)
				^ ((ABCD[ABCD_D] >> 24) & 0x000000ff);
		ABCD[ABCD_D] = (ABCD[ABCD_D] << 8) ^ ((T[0] >> 24) & 0x000000ff);
	}

	private static void BLOCK_XOR_CBC(int[] OUT_VALUE, int out_value_offset,
			int[] IN_VALUE1, int in_value1_offset, int[] IN_VALUE2,
			int in_value2_offset) {
		OUT_VALUE[out_value_offset + 0] = (in_value1_offset < IN_VALUE1.length ? IN_VALUE1[in_value1_offset + 0]
				: 0)
				^ (in_value2_offset < IN_VALUE2.length ? IN_VALUE2[in_value2_offset + 0]
						: 0);
		OUT_VALUE[out_value_offset + 1] = (in_value1_offset + 1 < IN_VALUE1.length ? IN_VALUE1[in_value1_offset + 1]
				: 0)
				^ (in_value2_offset + 1 < IN_VALUE2.length ? IN_VALUE2[in_value2_offset + 1]
						: 0);
		OUT_VALUE[out_value_offset + 2] = (in_value1_offset + 2 < IN_VALUE1.length ? IN_VALUE1[in_value1_offset + 2]
				: 0)
				^ (in_value2_offset + 2 < IN_VALUE2.length ? IN_VALUE2[in_value2_offset + 2]
						: 0);
		OUT_VALUE[out_value_offset + 3] = (in_value1_offset + 3 < IN_VALUE1.length ? IN_VALUE1[in_value1_offset + 3]
				: 0)
				^ (in_value2_offset + 3 < IN_VALUE2.length ? IN_VALUE2[in_value2_offset + 3]
						: 0);
	}
	...
	private static void KISA_SEED_Encrypt_Block_forCBC(int[] in, int in_offset,
			int[] out, int out_offset, KISA_SEED_KEY ks) {
	...
	private static void KISA_SEED_Decrypt_Block_forCBC(int[] in, int in_offset,
			int[] out, int out_offset, KISA_SEED_KEY ks) {
		int LR[] = new int[4]; // Iuput/output values at each rounds
		int T[] = new int[2]; // Temporary variables for round function F
		int K[] = ks.key_data; // Pointer of round keys
	...
	public static int SEED_CBC_init(KISA_SEED_INFO pInfo, KISA_ENC_DEC enc,
			byte[] pbszUserKey, byte[] pbszIV) {
	...
	public static int SEED_CBC_Process(KISA_SEED_INFO pInfo, int[] in,
			int inLen, int[] out, int[] outLen) {
		...
		if (KISA_ENC_DEC._KISA_ENCRYPT == pInfo.encrypt) {
			...
			while (nCurrentCount <= inLen) {
				BLOCK_XOR_CBC(out, out_offset, in, in_offset, pdwXOR,
						pdwXOR_offset);

				KISA_SEED_Encrypt_Block_forCBC(out, out_offset, out,
						out_offset, pInfo.seed_key);
			...
			while (nCurrentCount <= inLen) {
				KISA_SEED_Decrypt_Block_forCBC(in, in_offset, out, out_offset,
						pInfo.seed_key);

				BLOCK_XOR_CBC(out, out_offset, out, out_offset, pdwXOR,
						pdwXOR_offset);
			...
			Common.memcpy(pInfo.ivec, pdwXOR, pdwXOR_offset, BLOCK_SIZE_SEED);
			Common.memcpy(pInfo.cbc_last_block, out, out_offset
					- BLOCK_SIZE_SEED_INT, BLOCK_SIZE_SEED);
	...
	public static int SEED_CBC_Close(KISA_SEED_INFO pInfo, int[] out,
			int out_offset, int[] outLen) {
		int nPaddngLeng;
		int i;
		...
		if (KISA_ENC_DEC._KISA_ENCRYPT == pInfo.encrypt) {
			...
			for (i = pInfo.buffer_length; i < BLOCK_SIZE_SEED; i++) {
				Common.set_byte_for_int(pInfo.cbc_buffer, i,
						(byte) nPaddngLeng, ENDIAN);
			}
			BLOCK_XOR_CBC(pInfo.cbc_buffer, 0, pInfo.cbc_buffer, 0, pInfo.ivec,
					0);

			KISA_SEED_Encrypt_Block_forCBC(pInfo.cbc_buffer, 0, out,
					out_offset, pInfo.seed_key);
			...
			nPaddngLeng = Common.get_byte_for_int(pInfo.cbc_last_block,
					BLOCK_SIZE_SEED - 1, ENDIAN);

			if (nPaddngLeng > 0 && nPaddngLeng <= BLOCK_SIZE_SEED) {
				for (i = nPaddngLeng; i > 0; i--) {
					Common.set_byte_for_int(out, out_offset - i, (byte) 0x00,
							ENDIAN);
				}
	...
	public static byte[] SEED_CBC_Encrypt(byte[] pbszUserKey, byte[] pbszIV,
			byte[] message, int message_offset, int message_length) {
		...
		System.arraycopy(message, message_offset, pbszPlainText, 0,
				message_length);
		int nPlainTextLen = pbszPlainText.length;

		int nPlainTextPadding = BLOCK_SIZE_SEED
				- (nPlainTextLen % BLOCK_SIZE_SEED);
		...
		outlen = ((newpbszPlainText.length / BLOCK_SIZE_SEED))
				* BLOCK_SIZE_SEED_INT;
		...
		cdata = int32tochar_for_SEED_CBC(outbuf, nRetOutLeng[0]
				+ nPaddingLeng[0]);
		Common.arraycopy(pbszCipherText, cdata, nRetOutLeng[0]
				+ nPaddingLeng[0]);
	...
	public static byte[] SEED_CBC_Decrypt(byte[] pbszUserKey, byte[] pbszIV,
			byte[] message, int message_offset, int message_length) {
		KISA_SEED_INFO info = new KISA_SEED_INFO();
		int[] outbuf;
		int[] data;
		...
		System.arraycopy(message, message_offset, pbszCipherText, 0,
				message_length);
		...
		if (SEED_CBC_Close(info, outbuf, (nRetOutLeng[0]), nPaddingLeng) == 1) {
			...
			Common.arraycopy(pbszPlainText, cdata, nRetOutLeng[0]
					- nPaddingLeng[0]);
	...
	public static final class KISA_ENC_DEC {
		...
		public static final KISA_ENC_DEC KISA_ENCRYPT = new KISA_ENC_DEC(
				_KISA_ENCRYPT);
		public static final KISA_ENC_DEC KISA_DECRYPT = new KISA_ENC_DEC(
				_KISA_DECRYPT);
	...
	public static class Common {
		...
		public static void arraycopy_offset(byte[] dst, int dst_offset,
		...
		public static void arrayinit_offset(byte[] dst, int dst_offset,
		...
		public static void memcpy(int[] dst, int[] src, int src_offset,
				int length) {
		...
		public static void set_byte_for_int(int[] dst, int b_offset,
		...
		public static byte[] get_bytes_for_ints(int[] src, int offset,
				int ENDIAN) {
		...
		public static void byte_to_int(int[] dst, int dst_offset, byte[] src,
		...
		public static void int_to_byte_unit(byte[] dst, int dst_offset,
				int src, int ENDIAN) {
			if (ENDIAN == BIG_ENDIAN) {
		...
		public static void int_to_byte_unit_big_endian(byte[] dst,
		...
		// 1블럭(128비트 XOR)
		public static void BLOCK_XOR_PROPOSAL(int[] OUT_VALUE,
				int out_value_offset, int[] IN_VALUE1, int in_value1_offset,
	...
	public static int SeedRoundKey(KISA_SEED_INFO pInfo, KISA_ENC_DEC enc,
			byte[] pbszUserKey, byte[] pbszIV) {
		int ABCD[] = new int[4]; // Iuput/output values at each rounds(각 라운드
									// 입/출력)
		int T[] = new int[2]; // Temporary variable
	...
	public static void main(String[] args) {
		...
		byte pbUserKey[] = { (byte) 0x88, (byte) 0xE3, (byte) 0x4F,
				(byte) 0x8F, (byte) 0x08, (byte) 0x17, (byte) 0x79,
				(byte) 0xF1, (byte) 0xE9, (byte) 0xF3, (byte) 0x94,
				(byte) 0x37, (byte) 0x0A, (byte) 0xD4, (byte) 0x05, (byte) 0x89 };

		byte pbData[] = { (byte) 0x08, (byte) 0x09, (byte) 0x0A, (byte) 0x0B,
		...
		byte pbData2[] = { (byte) 0xD7, (byte) 0x6D, (byte) 0x0D, (byte) 0x18,
				(byte) 0x32, (byte) 0x7E, (byte) 0xC5, (byte) 0x62,
				(byte) 0xB1, (byte) 0x5E, (byte) 0x6B, (byte) 0xC3,
				(byte) 0x65, (byte) 0xAC, (byte) 0x0C, (byte) 0x0F };

		byte pbData3[] = { (byte) 0x00, (byte) 0x01, (byte) 0x02, (byte) 0x03,
				(byte) 0x04, (byte) 0x05, (byte) 0x06, (byte) 0x07,
				(byte) 0x08, (byte) 0x09, (byte) 0x0A, (byte) 0x0B,
				(byte) 0x0C, (byte) 0x0D, (byte) 0x0E, (byte) 0x0F,
				(byte) 0x00, (byte) 0x01 };

		byte bszIV[] = { (byte) 0x026, (byte) 0x08d, (byte) 0x066,
				(byte) 0x0a7, (byte) 0x035, (byte) 0x0a8, (byte) 0x01a,
				(byte) 0x081, (byte) 0x06f, (byte) 0x0ba, (byte) 0x0d9,
				(byte) 0x0fa, (byte) 0x036, (byte) 0x016, (byte) 0x025,
				(byte) 0x001 };
		...
		byte[] defaultCipherText = SEED_CBC_Encrypt(pbUserKey, bszIV, pbData,
				0, PLAINTEXT_LENGTH);

		byte[] PPPPP = SEED_CBC_Decrypt(pbUserKey, bszIV, defaultCipherText, 0,
				CIPHERTEXT_LENGTH);
		...
		System.arraycopy(pbszCipherText, pbszCipherText_offset, message, 0,
				CIPHERTEXT_LENGTH);
		...
		for (j = 0; j < nCipherTextLen - process_blockLeng;) {
			System.arraycopy(message, j, newpbszCipherText, 0,
					process_blockLeng);
			data = chartoint32_for_SEED_CBC(newpbszCipherText,
					process_blockLeng);
			SEED_CBC_Process(info, data, process_blockLeng, outbuf, nRetOutLeng);
			cdata = int32tochar_for_SEED_CBC(outbuf, nRetOutLeng[0]);
		...
		if (SEED_CBC_Close(info, outbuf, (nRetOutLeng[0]), nPaddingLeng) == 1) {
			cdata = int32tochar_for_SEED_CBC(outbuf, remainleng
					- nPaddingLeng[0]);

			byte[] newpbszPlainTexts = new byte[remainleng - nPaddingLeng[0]];

			Common.arraycopy(newpbszPlainTexts, cdata, remainleng
					- nPaddingLeng[0]);
