...
function getList(){
	...
	$.ajax({
 		type : 'GET',
 		url : '/rest/reportApi/CardProcessStatusListDept/'+$("#h_startAuthDate").val().replace(/-/g,'')+'/'+$("#h_endAuthDate").val().replace(/-/g,'')+'/'+$('#h_cardUserCode').val()+'/'+$('#h_financeNo').val()+'/all',
 		dataType : "json",
...
function cardDetailList(statusCode){
	...
	$.ajax({
 		type : 'GET',
 		url : "/rest/reportApi/CardProcessListDept/" + startAuthDate + "/" + endAuthDate + "/" + cardUserCode + "/" + financeNo + "/" + statusCode,
 		dataType : "json",
 		contentType: 'application/json',
 		beforeSend:function(){
 	    },
	...
	<div class="content">
		...
		<div class="titArea subTitArea">
			<h2><i class="icon iconStatus"></i> <span>부서별 카드처리현황</span></h2>
			<p class="location">Home > 레포트 > 부서별 카드처리현황</p>
		</div>
