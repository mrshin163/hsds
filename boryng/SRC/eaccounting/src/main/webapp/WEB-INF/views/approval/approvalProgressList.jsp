...
<script>
	$(document).ready(function () {
		...
		$("#dataArea").on('click', 'a', function(e) {
			e.preventDefault();
			openCalculateForm($.trim($(this).text()), null, 'approvalProgress');
		});
	});
	...
	function validForm() {
		...
		if (sDate == '' && eDate != '') {
			...
			bootbox.alert('종료일이 시작일 보다 빠릅니다.');
			return false;
		} else {
			$("#sDate").val(sDate);
			$("#eDate").val(eDate);
	...
	function getList(pageNo, flag){
		...
		$.ajax({
			type : 'GET',
			url : '/rest/approvalApi/approvalProgressList/' + $("#sDate").val() + '/' + $("#eDate").val() + '/' + pageNo,
			dataType : "json",
			beforeSend : function() {
...
<div class="content">
	...
	<div class="titArea subTitArea">
		...
		<p class="location">Home > 결재할 문서 > 진행중 문서</p>
	</div>
	<!-- 타이틀 E -->
	...
	<!-- 검색 내역 S -->
	<div class="srchList">
		<strong class="tit">진행중 리스트<em id="totalRow"></em>건</strong>
		<div class="innerBox">
