...
<script>
	$(document).ready(function () {
		...
		$("#dataArea").on('click', 'a', function(e) {
			e.preventDefault();
			openCalculateForm($.trim($(this).text()), null, 'complete');
		});
	});
	...
	function getList(pageNo, flag){
		...
		$.ajax({
			type : 'GET',
			url : '/rest/approvalApi/completeList/' + $("#sDate").val() + '/' + $("#eDate").val() + '/' + pageNo,
			dataType : "json",
			beforeSend : function() {
...
<div class="content">
	...
	<div class="titArea subTitArea">
		...
		<p class="location">Home > 결재상신 > 완료문서</p>
	</div>
	<!-- 타이틀 E -->
	...
	<!-- 검색 내역 S -->
	<div class="srchList">
		<strong class="tit">완료 리스트<em id="totalRow"></em>건</strong>
		<div class="innerBox">
