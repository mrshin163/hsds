   ...
   SELECT 'PHARM',
	 ...
	 FROM V_SYS_USER REQ, -- 상신자 정보
		  (SELECT COMPANY_CD, -- 대체자 정보
				  ...
				  DUTY_NM
			 FROM V_SYS_USER
			WHERE 1 = 1 AND COMPANY_CD = 'PHARM' AND USER_ID = '02116' -- 파라메터 : NREQ_USER_ID
																	  ) REP
	WHERE	 1 = 1
		  AND REQ.COMPANY_CD = REP.COMPANY_CD(+)
		  AND REQ.USER_ID = REP.USER_ID(+)
		  AND REQ.COMPANY_CD = 'PHARM'
		  AND REQ.USER_ID = '13044'						 -- 파라메터 : OWNER_ID (상신주체자)
...
SELECT
 ...
 , NULL					  /* CUSTOMER_NM */
 , NULL					  /* SAUP_NO */
 , '13044'				  /* USER_ID */ -- 파라메터 : OWNER_ID
 , (SELECT USER_NM FROM V_SYS_USER WHERE USER_ID='13044') /* USER_NM */ -- 파라메터 : OWNER_ID
 , NULL					  /* ACCOUNT2_CD */
 , NULL					  /* ACCOUNT2_NM */
 , NULL					  /* PRODUCT_CD */
 , NULL					  /* PRODUCT_NM */
