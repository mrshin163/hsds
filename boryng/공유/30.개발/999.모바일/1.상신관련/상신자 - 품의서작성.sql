...
-- 품의서ID 발행
-- ============================================================
   SELECT 'PHARM','APP'||TO_CHAR(SYSDATE,'YYYYMMDD')||LPAD(SEQ_APP.NEXTVAL,5,'0') FROM DUAL;
...
-- ============================================================
-- 품의서 기초작성 (결재미상신내역에서 선택한 파라메터 세팅필요)
-- ============================================================
INSERT INTO EACCOUNTING.EAC_APPROVAL (COMPANY_CD,
					  ...
					  UPD_USER_ID)
   SELECT         'PHARM',
		  'APP2015012800001', -- 파라메터 : 발행된 품의서ID
		  NULL,
		  NULL,
		  'T',
		  NULL,
	 ...
	 FROM V_SYS_USER REQ, -- 상신자 정보
		  (SELECT COMPANY_CD, -- 대체자 정보
				  ...
				  DUTY_NM
			 FROM V_SYS_USER
			WHERE 1 = 1 AND COMPANY_CD = 'PHARM' AND USER_ID = '09158' -- 파라메터 : NREQ_USER_ID
																	  ) REP
	WHERE	 1 = 1
		  AND REQ.COMPANY_CD = REP.COMPANY_CD(+)
		  AND REQ.USER_ID = REP.USER_ID(+)
		  AND REQ.COMPANY_CD = 'PHARM'
		  AND REQ.USER_ID = '09158'  -- 파라메터 : OWNER_ID (상신주체자)
...
-- ============================================================
-- 품의내역서 기초작성 (결재미상신내역에서 선택한 매입건의 6개 키로 관련자료 조회
-- ============================================================
...
SELECT
   'PHARM'				  /* 회사코드 */
 , 'APP2015012800001'     /* 품의서ID */  -- 파라메터 : 발행된 품의서ID
 , (SELECT COUNT(*)+1 FROM EAC_APPROVAL_ITEM WHERE COMPANY_CD='PHARM' AND APPROVAL_ID='APP2015012900011')                  /* 품의내역 일련번호 */
 , NULL					  /* S_APPROVAL_ID */
 , NULL					  /* S_APPROVAL_SEQ */
 , NULL					  /* CUSTOMER_CD */
 , NULL					  /* CUSTOMER_NM */
 , NULL					  /* SAUP_NO */
 , '09158'				  /* USER_ID */ -- 파라메터 : OWNER_ID
 , (SELECT USER_NM FROM V_SYS_USER WHERE USER_ID='13044' AND COMPANY_CD='PHARM') /* USER_NM */ -- 파라메터 : OWNER_ID
 , NULL					  /* ACCOUNT2_CD */
 , NULL					  /* ACCOUNT2_NM */
 , NULL					  /* PRODUCT_CD */
 , NULL					  /* PRODUCT_NM */
 ...
 , '09158'				  /* REG_USER_ID */ -- 고정 파라메터 : 로그인한 사용자ID
 , NULL					  /* UPD_DTTM */
 , NULL					  /* UPD_USER_ID */ 
FROM CARD_AQ_TMP C, 
	 (
...
WHERE 1=1
  ...
  AND C.CARD_NUM = '' -- 파라메터 : 카드번호
  AND C.AUTH_DATE = '' -- 파라메터 : 승인일자
  AND C.AUTH_NUM  = '' -- 파라메터 : 승인번호
  AND C.GEORAE_STAT = '' -- 파라메터 : 거래상태
  AND C.REQUEST_AMOUNT = '' -- 파라메터 : 금액합계
  AND C.GEORAE_COLL = '' -- 파라메터 : 매입추심번호
