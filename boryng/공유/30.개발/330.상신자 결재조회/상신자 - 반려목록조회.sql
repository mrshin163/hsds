...
-- ====================================
  ...
  SELECT A.COMPANY_CD               AS COMPANY_CD,
         ...
         MAX (ALM.AP_DEPT_NM)       AS REJ_AP_DEPT_NM,
         MAX (ALM.AP_USER_NM)       AS REJ_AP_USER_NM,
         MAX (ALM.APR_DATE)         AS REJ_DATE,
         MAX (ALM.APR_COMMENT)      AS REJ_COMMENT
    FROM EAC_APPROVAL A,
   ...
   WHERE     1 = 1
         ...
         AND A.APPROVAL_STATUS_CD IN ('NREJ','SREJ')                    --  고정
         AND A.DEL_YN = 'N'                                             -- 고정
         ...
         --   AND (A.RQ_USER_ID = '10108' OR A.RRQ_USER_ID='10108')             -- 검색조건 : 본인시 세션사용자ID를 넣을 것
